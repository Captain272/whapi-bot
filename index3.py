...
#             elif text_body == "shutdown":
#                 bot_active = False
#                 response_body = "Application shut down successfully."
#             elif text_body == "hotel number":
#                 response_body = f"The current hotel employee number is {ALLOWED_HOTEL_EMPLOYEE_NUMBER}."
#             elif text_body.startswith("change hotel number"):
#                 new_num = text_body.replace("change hotel number", "").strip()
#                 if new_num:
...
#             elif text_body == "airlines number":
#                 response_body = f"The current airlines number is {ALLOWED_DUTY_MANAGER_NUMBER}."
#             elif text_body.startswith("change airlines number"):
#                 new_num = text_body.replace("change airlines number", "").strip()
#                 if new_num:
...
#             elif text_body == "view rooms":
#                 response_body = f"Available rooms: {ROOMS}"

#             elif text_body.startswith("change rooms"):
#                 new_rooms = text_body.replace("change rooms", "").strip()
#                 ROOMS = new_rooms
#                 update_env_var("ROOMS", new_rooms)
#                 response_body = f"Rooms updated to: {new_rooms}"
#             elif "help" in text_body:
#                 response_body = (
...
cloudwatch_handler = watchtower.CloudWatchLogHandler(log_group="FalaskAppLogs", stream_name="FlaskAppStream")
...
def handle_new_messages(request_data: Dict[str, Any]):
    ...
    for message in messages:
        ...
        # Handle Flight Delay Group
        if chat_name == "UAT - SIA Flight Delay Group Chat" and bot_active and sender == ALLOWED_DUTY_MANAGER_NUMBER:
        ...
        elif chat_name == "UAT - SIA Internal Group":
 
            if text_body == "rps start":
                bot_active = True
                response_body = "Application started successfully."
            elif text_body == "shutdown":
                bot_active = False
                response_body = "Application shut down successfully."
            elif text_body == "rps hotel number":
                response_body = f"The current hotel employee number is {ALLOWED_HOTEL_EMPLOYEE_NUMBER}."
            elif text_body.startswith("rps change hotel number"):
                new_num = text_body.replace("rps change hotel number", "").strip()
                if new_num:
                    ALLOWED_HOTEL_EMPLOYEE_NUMBER = new_num
                    response_body = f"Hotel employee number updated to {new_num}."
                    update_env_var("ALLOWED_HOTEL_EMPLOYEE_NUMBER", new_num)
            elif text_body == "rps airlines number":
                response_body = f"The current airlines number is {ALLOWED_DUTY_MANAGER_NUMBER}."
            elif text_body.startswith("rps change airlines number"):
                new_num = text_body.replace("rps change airlines number", "").strip()
                if new_num:
                    ALLOWED_DUTY_MANAGER_NUMBER = new_num
                    response_body = f"Airlines number updated to {new_num}."
                    update_env_var("ALLOWED_DUTY_MANAGER_NUMBER", new_num)

            elif text_body == "rps view rooms":
                response_body = f"Available rooms: {ROOMS}"
        ...
        # Send response
        if response_body:
            send_whapi_request('messages/text', {'to': message.get('chat_id'), 'body': response_body+" old"})
            end_ts = time.time()
            elapsed_seconds = end_ts - current_ts
            logger.info(f"Elapsed time in seconds:{current_ts},{end_ts},{elapsed_seconds}")
            # send_whapi_request('messages/text', {'to': message.get('chat_id'), 'body': f"Response time : {elapsed_seconds} {end_ts} sec"})
